...
### FUNCTION
### get the list of changed files, non-binary, that contain author tag
function get_files
...
function get_authors_commit
    git shortlog -s $commit_range $argv | sed "s/^[[:space:][:digit:]]*//" \
                ...
                | sort | uniq \
                | sed '/Frederic Tessier/d'
end

### echo commit range
echo
echo "COMMITS: ($commit_range)"
for c in (git log --pretty=format:'%h  %<(51)%s  %an' $commit_range)
    echo "   $c"
end
...
set files_to_check (git diff --name-only $commit_range)
for f in $files_to_check
